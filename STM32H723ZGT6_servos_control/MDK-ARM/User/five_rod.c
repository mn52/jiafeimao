...
float Count_pack_alpha(float horizon,float height){
	...
	if((alpha_count_a>-270&&alpha_count_a<-90)||(alpha_count_a>0||alpha_count_a<270))return alpha_count_a;
    if((alpha_count_b>-270&&alpha_count_b<-90)||(alpha_count_b>0||alpha_count_b<270))return alpha_count_b;
	return 90.0;
}
float Count_pack_beta(float horizon,float height){
	...
	if(beta_count_a>-90&&beta_count_a<90)return beta_count_a;
    if(beta_count_b>-90&&beta_count_b<90)return beta_count_b;
...
void Servos_left(float horizon,float height){
    ...
    if(left_a<0)PCA9685_SetServoAngle(0,-left_a-90);
    else PCA9685_SetServoAngle(0,270-left_a);
     
    left_b=Count_pack_beta(horizon,height);
    PCA9685_SetServoAngle(1,90-left_b);
   
}

void Servos_right(float horizon,float height){

    right_a=Count_pack_beta(horizon,height);
    PCA9685_SetServoAngle(14,90+3-right_a);
    right_b=Count_pack_alpha(horizon,height);
    if(right_b<0)PCA9685_SetServoAngle(15,-right_b-90);
    else PCA9685_SetServoAngle(15,270-right_b);
